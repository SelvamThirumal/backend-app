...
function isDuplicateNutrient(nutrients, nutrient_id) {
  for (const nutrient of nutrients) {
    if (nutrient.nutrient_id === nutrient_id) return true;
  }
  return false;
}
...
class FoodController {
  static async getFood(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal server error"));
    }
  }
  ...
  static async createFood(req, res) {
    const { name, description, meals, quantity, serving, nutrients, calories } =
      req.body;
    ...
    if (!calories)
      return res
        .status(400)
        .json(ApiResponse.error("{ calories } is required"));

    if (!Array.isArray(meals))
      return res
        .status(400)
        .json(ApiResponse.error("{ meals } excepts only array"));

    if (meals.length === 0)
      return res
        .status(400)
        .json(
          ApiResponse.error("At least one meal should be pass in { meals }")
        );
    ...
    if (isDuplicateMeals)
      return res
        .status(400)
        .json(ApiResponse.error("two or more meals are same"));

    if (!quantity)
      return res
        .status(400)
        .json(ApiResponse.error("{ quantity } is required"));
    ...
    if (!nutrients)
      return res
        .status(400)
        .json(ApiResponse.error("{ nutrients } is required"));

    if (!Array.isArray(nutrients))
      return res
        .status(400)
        .json(ApiResponse.error("{ nutrients } must be an array"));

    if (nutrients.length === 0)
      return res
        .status(400)
        .json(ApiResponse.error("{ nutrients } array cannot be empty"));
    ...
    try {
      ...
      for (const meal of meals) {
        if (!mongoose.Types.ObjectId.isValid(meal))
          return res
            .status(400)
            .json(
              ApiResponse.error("Invalid meals { id } try checking from meals")
            );
        ...
        if (!checkMealExistence)
          return res
            .status(400)
            .json(ApiResponse.error("Meal does not exist with id: " + meal));
      ...
      for (const nutrient of nutrients) {
        if (isDuplicateNutrient(nutrientList, nutrient.nutrient_id))
          return res
            ...
            .json(
              ApiResponse.error(
                "Duplicate nutrient at index of " +
                  (nutrients.indexOf(nutrient) + 1)
              )
            );
        ...
        if (!nutrient.nutrient_id)
          return res
            .status(400)
            .json(ApiResponse.error("Nutrient { nutrient_id } is required"));
        if (!nutrient.quantity)
          return res
            .status(400)
            .json(ApiResponse.error("Nutrient { quantity } is required"));
        ...
        if (!mongoose.Types.ObjectId.isValid(nutrient.nutrient_id))
          return res
            ...
            .json(
              ApiResponse.error(
                "Invalid nutrient id at index of " +
                  (nutrients.indexOf(nutrient) + 1)
              )
            );

        const checkNutrientExistence = await Nutrient.findById(
        ...
        if (!checkNutrientExistence)
          return res
            ...
            .json(
              ApiResponse.error(
                "Nutrient is invalid at index of " +
                  (nutrients.indexOf(nutrient) + 1) +
                  " try fetching nutrients"
              )
            );
      ...
      if (isExistingFood)
        return res
          ...
          .json(
            ApiResponse.error(
              "Food already exists, please use a different name"
            )
          );
      ...
      const food = await Food.findById(createdFood._id)
        .select("-__v -updatedAt -createdAt")
        .populate({
          path: "meals",
          select: "name description start_time end_time",
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Food created successfully", food));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal server error"));
  ...
  static async updateFood(req, res) {
    const { id } = req.params;
    const { name, description, meals, quantity, serving, calories, nutrients } =
      req.body;

    if (!id)
      return res
        .status(400)
        .json(ApiResponse.error("food id is required to update"));
    ...
    if (meals) {
      ...
      if (meals.length === 0)
        return res
          .status(400)
          .json(ApiResponse.error("{ meals } array cannot be empty"));
    ...
    try {
        ...
        for (const meal of meals) {
          if (!mongoose.Types.ObjectId.isValid(meal))
            return res
              ...
              .json(
                ApiResponse.error(
                  "Invalid meals { id } try checking from meals"
                )
              );
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Food updated successfully", food));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal Server Error"));
  ...
  static async deleteFood(req, res) {
    ...
    if (!mongoose.Types.ObjectId.isValid(id))
      return res.status(400).json(ApiResponse.error("Invalid food id"));
    try {
      const food = await Food.findByIdAndDelete(id).select(
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Food deleted successfully", food));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal server error"));
