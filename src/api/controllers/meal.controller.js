...
class MealController {
  ...
  static async createMeal(req, res) {
    ...
    if (!name || !description || !start_time || !end_time)
      return res
        ...
        .json(
          ApiResponse.error(
            "All fields are required { name,description,start_time,end_time }"
          )
        );
    ...
    try {
      ...
      const fetchedMeal = await Meal.findById(createdMeal).select(
        "-__v -updatedAt -createdAt"
      );

      return res
        .status(200)
        .json(ApiResponse.success("Meal created successfully", fetchedMeal));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal server error"));
  ...
  static async updateMeal(req, res) {
    ...
    try {
      ...
      const updatedMeal = await Meal.findById(id).select(
        "-__v -updatedAt -createdAt"
      );
  ...
  static async deleteMeal(req, res) {
    ...
    try {
      ...
      const deletedMeal = await Meal.findByIdAndDelete(id).select(
        "-__v -updatedAt -createdAt"
      );
      return res
        .status(200)
        .json(ApiResponse.success("Meal deleted successfully", deletedMeal));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal server error"));
