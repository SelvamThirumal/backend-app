...
class ProfileController {
  static async getProfile(req, res) {
    ...
    try {
      ...
      if (!existingUserProfile) {
        return res
          .status(400)
          .json(ApiResponse.error("Profile does not exist,try creating it"));
      }
      return res
        .status(200)
        .json(
          ApiResponse.success("User found successfully", existingUserProfile)
        );
    } catch (e) {
      return res
        .status(500)
  ...
  static async createProfile(req, res) {
    ...
    if (!target_weight)
      return res
        .status(400)
        .json(ApiResponse.error("{ targeted_weight } is required"));
    ...
    if (!activity_level)
      return res
        .status(400)
        .json(ApiResponse.error("{ activity_level } is required"));
    ...
    if (!diet_plans)
      return res
        .status(400)
        .json(ApiResponse.error("{ diet_plans } is required"));

    if (!Array.isArray(diet_plans))
      return res
        .status(400)
        .json(ApiResponse.error("{ diet_plans } must be an array"));

    if (!health_issues)
      return res
        .status(400)
        .json(ApiResponse.error("{ health_issues } is required"));

    if (!Array.isArray(health_issues))
      return res
        .status(400)
        .json(ApiResponse.error("{ health_issues } must be an array"));
    ...
    try {
      ...
      for (let issue of health_issues) {
        if (mongoose.Types.ObjectId.isValid(issue) === false)
          return res
            .status(400)
            .json(ApiResponse.error("Invalid Health Issue Id"));
        const healthIssue = await HealthIssue.findById(issue);
        if (!healthIssue)
          return res
            .status(400)
            .json(ApiResponse.error("Health Issue does not exist"));
      ...
      if (!createdUser) {
        return res
          .status(400)
          .json(ApiResponse.error("Error creating profile"));
      }

      //update user status to 1
      if (user.status_id < 1) {
        const isStatusUpdated = await User.findByIdAndUpdate(user_id, {
          status_id: 1,
        });
        ...
        if (!isStatusUpdated) {
          ...
          return res
            .status(400)
            .json(ApiResponse.error("Error updating user status  "));
        }
      }
    ...
    try {
      ...
      if (diet_plans) {
        ...
        if (diet_plans.length === 0) {
          for (let plan of diet_plans) {
            if (mongoose.Types.ObjectId.isValid(plan) === false)
              return res
                .status(400)
                .json(ApiResponse.error("Invalid Diet Plan Id"));
            const dietPlan = await DietPlan.findById(plan);
            if (!dietPlan)
              return res
                .status(400)
                .json(ApiResponse.error("Diet Plan does not exist"));
        ...
        if (existingUserProfile.diet_plans.length === 0) {
          existingUserProfile.diet_plans = await getDietPlans(
            existingUserProfile.health_issues
          );
        }
      }
      ...
      let updatedProfile = await UserProfile.findById(
  ...
  static async deleteProfile(req, res) {
    ...
    try {
      ...
      if (!existingProfile) {
        return res
          .status(400)
          .json(ApiResponse.error("Profile does not exist"));
      }
      ...
      if (!deletedProfile) {
        return res
          .status(400)
          .json(ApiResponse.error("Error deleting profile"));
      }
      ...
      if (!isUpdated) {
        return res
          .status(400)
          .json(
            ApiResponse.error("Profile Deleted and Error updating user status")
          );
      }

      return res
        .status(200)
        .json(ApiResponse.success("Profile deleted successfully"));
    } catch (e) {
      return res
        .status(500)
        .json(ApiResponse.error(e.message || "Internal Server Error"));
