...
dotenv.config();
import referral from "../../utils/referral_code_generator.js";
...
const client = new OAuth2Client(
  "306367459055-bmlma1mamhcdq6pk6ca6q2vhas2jp5du.apps.googleusercontent.com"
);
class AuthController {
  static async google(req, res) {
    ...
    try {
      ...
      if (!email_verified) {
        return res
          .status(400)
          .json(ApiResponse.error("Google account is not verified"));
      }

      const googleUserId = sub;
      let user = await User.findOne({ email }).select("-__v");

      let res_message = "User logged in successfully";

      const referralCode = referral.generateReferralCode(name);
      ...
      return res
        .status(200)
        .json(ApiResponse.success(res_message, updatedUser));
    } catch (error) {
      console.error(error);
      return res
        .status(400)
  ...
  static async registerAdmin(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json(ApiResponse.success("User created successfully", updatedUser));
    } catch (error) {
      console.error(error);
  ...
  static async emailAndPasswordLogin(req, res) {
    ...
    try {
      ...
      const updatedUser = await User.findById(existingUser.id).select(
        "-__v -password"
      );
  ...
  static async verify(req, res) {
    ...
    try {
      ...
      const existingUser =
        await User.findById(user_id).select("-__v -password");

      res.cookie("token", existingUser.token, { httpOnly: true });

      return res
        .status(200)
        .json(ApiResponse.success("User Verified", existingUser));
    } catch (error) {
      res.clearCookie("token");
      return res
        .status(400)
  ...
  static async logout(req, res) {
    ...
    try {
      ...
      if (existingUser.token !== token)
        return res
          .status(400)
          .json(ApiResponse.error("Session Expired, Please login again"));
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Logout successful", true));
    } catch (error) {
      return res
        .status(500)
        .json(ApiResponse.error(error.message || "Internal Server Error"));
