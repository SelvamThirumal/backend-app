...
class HealthIssueController {
  ...
  static async getAllHealthIssues(req, res) {
    try {
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Health Issues List fetched", result));
    } catch (error) {
      return res
        .status(400)
  ...
  static async getHealthIssueById(req, res) {
    ...
    try {
      ...
      if (!isValidId)
        return res
          .status(400)
          .json(ApiResponse.error("Invalid HealthIssue Id"));
      ...
      if (result !== null) {
        return res
          .status(200)
          .json(ApiResponse.success("Health Issue found", result));
      } else {
        return res
          .status(400)
          .json(ApiResponse.error("No Health issue found with this id"));
      }
    } catch (error) {
      return res
        .status(400)
  ...
  static async addHealthIssue(req, res) {
    ...
    if (name === undefined || name === "")
      return res
        .status(400)
        .json(ApiResponse.error("'name' field is required"));

    if (risk_factor === undefined || risk_factor === "")
      return res
        .status(400)
        .json(ApiResponse.error("'risk_factor' field is required"));

    if (cause === undefined || cause === "")
      return res
        .status(400)
        .json(ApiResponse.error("'cause' field is required"));

    if (treatment === undefined || treatment === "")
      return res
        .status(400)
        .json(ApiResponse.error("'treatment' field is required"));
    ...
    try {
      ...
      return res
        .status(200)
        .json(ApiResponse.success("Health Issue Added", result));
    } catch (error) {
      return res
        .status(400)
        .json(ApiResponse.error(error.message || "Internal Server Error"));
  ...
  static async updateHealthIssue(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json(
          ApiResponse.success("Health Issue Updated successfully", healthIssue)
        );
    } catch (error) {
      return res
        .status(400)
  ...
  static async deleteHealthIssue(req, res) {
    ...
    try {
      ...
      if (healthIssue == null)
        return res
          .status(400)
          .json(ApiResponse.error("Health issue is not available"));

      return res.status(200).json({
        status: ApiResponse.success,
        message: ("Health Issue deleted successfully", healthIssue),
      });
    } catch (error) {
      return res
        .status(400)
