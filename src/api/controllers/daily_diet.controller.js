...
const getMeal = (mealName) =>
  meals.find((meal) => meal.name.toLowerCase() === mealName);

const breakfastMeal = getMeal('breakfast');
const morningSnacksMeal = getMeal('morning_snacks');
const lunchMeal = getMeal('lunch');
const eveningSnacksMeal = getMeal('evening_snacks');
const dinnerMeal = getMeal('dinner');

if (!breakfastMeal || !morningSnacksMeal || !lunchMeal || !eveningSnacksMeal || !dinnerMeal) {
  throw new Error('One or more meal types are missing in the Meal collection');
}
